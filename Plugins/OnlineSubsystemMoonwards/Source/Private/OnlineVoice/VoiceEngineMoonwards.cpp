...
namespace VoiceEngineUtilitiesMW
{
...
FVoiceEngineMoonwards ::FVoiceEngineMoonwards()
...
FVoiceEngineMoonwards::FVoiceEngineMoonwards(IOnlineSubsystem* InSubsystem) :
	OnlineInstanceName(NAME_None),
	VoiceCapture(nullptr),
	VoiceEncoder(nullptr),
	OwningUserIndex(INVALID_INDEX),
...
{
	FCoreUObjectDelegates::PostLoadMapWithWorld.AddRaw(this, &FVoiceEngineMoonwards::OnPostLoadMap);
...
void FVoiceEngineMoonwards::VoiceCaptureUpdate() const
{
...
void FVoiceEngineMoonwards::StartRecording() const
{
...
void FVoiceEngineMoonwards::StopRecording() const
{
...
void FVoiceEngineMoonwards::StoppedRecording() const
{
	UE_LOG_ONLINE_VOICEENGINE(VeryVerbose, TEXT("VOIP StoppedRecording"));
}

bool FVoiceEngineMoonwards::Init(int32 MaxLocalTalkers, int32 MaxRemoteTalkers)
{
	bool bSuccess = false;
...
uint32 FVoiceEngineMoonwards::StartLocalVoiceProcessing(uint32 LocalUserNum) 
{
...
uint32 FVoiceEngineMoonwards::StopLocalVoiceProcessing(uint32 LocalUserNum) 
{
	uint32 Return = ONLINE_FAIL;
	if (IsOwningUser(LocalUserNum))
	{
...
uint32 FVoiceEngineMoonwards::RegisterLocalTalker(uint32 LocalUserNum)
{
	if (!VoiceCapture.IsValid())
	{
...
uint32 FVoiceEngineMoonwards::UnregisterLocalTalker(uint32 LocalUserNum)
{
	if (IsOwningUser(LocalUserNum))
	{
		OwningUserIndex = INVALID_INDEX;
...
uint32 FVoiceEngineMoonwards::UnregisterRemoteTalker(const FUniqueNetId& UniqueId)
{
...
uint32 FVoiceEngineMoonwards::GetVoiceDataReadyFlags() const
{
...
uint32 FVoiceEngineMoonwards::ReadLocalVoiceData(uint32 LocalUserNum, uint8* Data, uint32* Size, uint64* OutSampleCount)
{
	check(*Size > 0);
...
uint32 FVoiceEngineMoonwards::SubmitRemoteVoiceData(const FUniqueNetIdWrapper& RemoteTalkerId, uint8* Data, uint32* Size, uint64& InSampleCount)
{
...
void FVoiceEngineMoonwards::TickTalkers(float DeltaTime)
{
...
void FVoiceEngineMoonwards::Tick(float DeltaTime)
{
...
void FVoiceEngineMoonwards::GenerateVoiceData(USoundWaveProcedural* InProceduralWave, int32 SamplesRequired, const FUniqueNetId& TalkerId)
{
...
void FVoiceEngineMoonwards::OnAudioFinished()
{
	for (FRemoteTalkerData::TIterator It(RemoteTalkerBuffers); It; ++It)
	{
		FRemoteTalkerDataImpl& RemoteData = It.Value();
...
void FVoiceEngineMoonwards::OnPostLoadMap(UWorld*)
{
	for (FRemoteTalkerData::TIterator It(RemoteTalkerBuffers); It; ++It)
	{
		FRemoteTalkerDataImpl& RemoteData = It.Value();
...
FString FVoiceEngineMoonwards::GetVoiceDebugState() const
{
	FString Output;
...
bool FVoiceEngineMoonwards::Exec(UWorld* InWorld, const TCHAR* Cmd, FOutputDevice& Ar)
{
...
Audio::FPatchOutputStrongPtr FVoiceEngineMoonwards::GetMicrophoneOutput()
{
...
Audio::FPatchOutputStrongPtr FVoiceEngineMoonwards::GetRemoteTalkerOutput()
{
	return AllRemoteTalkerAudio.AddNewOutput(4096 * 2, 1.0f);
}

float FVoiceEngineMoonwards::GetMicrophoneAmplitude(int32 LocalUserNum)
{
	if (VoiceCapture.IsValid())
	{
...
float FVoiceEngineMoonwards::GetIncomingAudioAmplitude(const FUniqueNetIdWrapper& RemoteUserId)
{
...
uint32 FVoiceEngineMoonwards::SetRemoteVoiceAmplitude(const FUniqueNetIdWrapper& RemoteTalkerId, float InAmplitude)
{
...
bool FVoiceEngineMoonwards::PatchRemoteTalkerOutputToEndpoint(const FString& InDeviceName, bool bMuteInGameOutput /*= true*/)
{
...
void FVoiceEngineMoonwards::DisconnectAllEndpoints()
{
	static IConsoleVariable* MuteAudioEngineOutputCVar = IConsoleManager::Get().FindConsoleVariable(TEXT("voice.MuteAudioEngineOutput"));
	check(MuteAudioEngineOutputCVar);
...
int32 FVoiceEngineMoonwards::GetMaxVoiceRemainderSize()
{
	return MAX_VOICE_REMAINDER_SIZE;
}

void FVoiceEngineMoonwards::CreateSerializeHelper()
{
	if (SerializeHelper == nullptr)
	{
		SerializeHelper = new FVoiceSerializeHelperMW(this);
	}
}

IOnlineSubsystem* FVoiceEngineMoonwards::GetOnlineSubSystem()
{
...
bool FVoiceEndpointMW::OnProcessAudioStream(Audio::FAlignedFloatBuffer& OutputBuffer)
{
	...
	if (OutputPatch.IsValid() && OutputPatch->GetNumSamplesAvailable() >= NumFrames * NumChannelsComingIn)
	{
		if (PlatformDeviceInfo.NumChannels != NumChannelsComingIn)
		{
			...
			VoiceEngineUtilitiesMW::DownmixBuffer(DownmixBuffer.GetData(), OutputBuffer.GetData(), NumFrames, NumChannelsComingIn, PlatformDeviceInfo.NumChannels);
		}
		else
		{
