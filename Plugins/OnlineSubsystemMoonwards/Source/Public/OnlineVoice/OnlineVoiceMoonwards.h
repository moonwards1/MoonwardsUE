 ...
 * 
 */
class ONLINESUBSYSTEMMOONWARDS_API FOnlineVoiceMoonwards : public IOnlineVoice
{
public:
protected:
...
PACKAGE_SCOPE:
		...
		FOnlineVoiceMoonwards(IOnlineSubsystemPtr OnlineSubsystem) :
		OnlineSubsystem(OnlineSubsystem.Get()),
		// SessionInt(NULL),
		IdentityInt(NULL),
		VoiceEngine(NULL),
...
public:
	...
	/** Constructor */
		FOnlineVoiceMoonwards(class IOnlineSubsystem* InOnlineSubsystem);

	/** Virtual destructor to force proper child cleanup */
	virtual ~FOnlineVoiceMoonwards();
...
typedef TSharedPtr<FOnlineVoiceMoonwards, ESPMode::ThreadSafe> FOnlineVoiceMoonwardsPtr;
